...
{	
	...
	int n = x * y * 18;
	OpenGLSquare square;
	SquareMesh s1 = SquareMesh(x,y,1);
	s1.normalizeVertices();
	//s1.printVertices();
	cout << "GOTOWE" << endl;
	getchar();
	if (!initOpenGL())
	...
	glBufferData(GL_ARRAY_BUFFER, n*sizeof(GLfloat), s1.getVertices(), GL_STATIC_DRAW);	// copy the data from CPU to GPU
